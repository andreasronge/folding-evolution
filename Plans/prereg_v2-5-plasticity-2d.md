...
**Status:** READY-TO-LAUNCH v7.1 (post-compute amendment; codex-v7-review FAIL (2 P1 + 1 P2) discharged in-place; scope re-targeted to budget=40 primary-confirmatory cell; supporting cells {5, 10, 20} deferred to §2d-supplemental; "matched eval-time compute" corrected to "matched `plasticity_budget`"; partial-run provenance corrected from 8 to 18 runs with quarantine + post-sweep determinism check) · target commit `5512815` (v6 engineering commit was `ee44b1c`; v7 scope-amendment was `fb179ce`) · 2026-04-24.
...
3. **Post-sweep determinism check (mandatory, v7):** compute SHA-256 of `final_population.npz` and `result.json` at each of the 4 overlap seeds (20..23) between 2026-04-23 partial data and the new 2026-04-24 sweep. Code is unchanged between ee44b1c and the v7 target SHA, so overlap-seed outputs MUST be byte-identical; any divergence flags Row 6 SWAMPED ("determinism-failure" reason) and halts the chronicle pending investigation. Implemented as an analyzer-layer check invoked before the paired-bootstrap confirmatory routine.
