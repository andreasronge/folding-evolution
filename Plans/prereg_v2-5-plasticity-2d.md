...
**Launch attempt at commit `ee44b1c` (2026-04-23) timed out at 10800s (SIGTERM exit 143) with 8 of 80 runs completed.** Partial output at `experiments/output/2026-04-23/v2_5_plasticity_2d/`; `queue.status.json` entry `v2_5_plasticity_2d` is status=timeout (preserved as historical record; not renamed — only a single attempt under this id).

Observed per-run walls — linear-in-budget scaling holds, but ~8× over the v6 projection:

| budget | completed / 20 | observed wall       | v6 projection | over |
|--------|----------------|---------------------|---------------|------|
| 5      | 5              | ~3220s (54 min)     | ~7 min        | ~8×  |
| 10     | 3              | ~6100s (102 min)    | ~15 min       | ~7×  |
| 20     | 0              | (~3.3h projected)   | ~22 min       | —    |
| 40     | 0              | (~6.7h projected)   | ~30 min       | —    |
...
- The 2026-04-23 partial data (5 at budget=5, 3 at budget=10, 0 at 20/40) is NOT consumed by §2d-primary analysis. It may inform §2d-supplemental at chronicle time if re-run at those budgets later.
