...
**H-SWAMPED (infrastructure):** `initial_population_canonical_count > 0` on any run (sf=0.0 invariant violated) OR **hash collision** between any §2d random-sample cell and any §2a / §2c reused cell (dedup accidentally short-circuits one of the 80 new runs) OR **seed-integrity pre-check fails** (any missing/duplicated/extra seed in either the plastic budget=40 reference subset from §2c or the random-sample budget=40 cell per Setup § "Seed-integrity pre-check") OR **mechanism-sanity pre-check fails** (any random-sample cell's `delta_final` distribution violates uniform-`[-b, +b]` support bounds, OR per-run k-draw summary shows draws outside `[−b, +b]` support or collapsed std per Setup § "Mechanism-sanity pre-check" — both support-bound and draw-distribution checks are mandatory at launch per codex-v1 P1-5 correction). Routing: stop and inspect; no routing until infrastructure check passes.
...
- **Supporting-cell cross-mechanism per-cell point estimates and 98.333% seed-bootstrap CIs at plasticity_budget ∈ {5, 10, 20}** — reported descriptively; the paired point-estimate signs `F̂_plastic(b) − F̂_random(b)` at these budgets are reported per §26 chronicle-time discipline. If a sign flips or a dramatic cross-mechanism pattern appears at supporting budgets, the chronicle flags this as a §2b grid-miss discovery and the follow-up prereg enumerates it. Supporting cells DO NOT gate routing at chronicle time.
...
- **Sweep file:** `experiments/chem_tape/sweeps/v2/v2_5_plasticity_2d.yaml` (to be created; see Status-transition checklist item 5).
- **Arms / conditions:** Arm A only (direct GP). BP_TOPK EXCLUDED per §v2.5-plasticity-1a structural `R_fit` ceiling caveat (same as §v2.5-plasticity-2a/2c). **4 new `random_sample_threshold` cells at matched plasticity_budget ∈ {5, 10, 20, 40}:**
  - `arm=A, plasticity_enabled=true, plasticity_mechanism=random_sample_threshold, plasticity_budget ∈ {5, 10, 20, 40}, seed_fraction=0.0, generations=1500, pop_size=512, mutation_rate=0.03, tournament_size=3, elite_count=2, crossover_rate=0.7, plasticity_delta=1.0, plasticity_train_fraction=0.75, tape_length=32, n_examples=64, holdout_size=256, alphabet=v2_probe, task=sum_gt_10_AND_max_gt_5, disable_early_termination=true, dump_final_population=true, backend=mlx` × 4 budgets × 20 seeds = **80 new runs**.
  - **`rank1_op_threshold` cells at budgets {5, 10, 20, 40} are REUSED from §2a + §2c at commits `7361631` (budget=5 pooled n=40 — confirmatory test uses only the seeds-20..39 subset, n=20) and `06e8732` (budgets {10, 20, 40} at n=20 each).** NOT re-run here. The 20 per-winner artifacts for plastic budget=40 seeds 20..39 are on disk in `experiments/output/` under the §2c sweep output directory (exact path to be named in sweep YAML).
  - No new frozen-control cell needed (§2d is a cross-mechanism comparison, not a plasticity-vs-frozen comparison).
- **Seeds:** **seeds 20..39** on all 4 new `random_sample_threshold` cells. Identical seed block to §2a's primary cell and §2c's plastic cells {10, 20, 40}. The paired-seed structure on shared seeds 20..39 at budget=40 is the internal-control contrast (principle 1).
- **Fixed params:** pop=512, gens=1500, mr=0.03, crossover_rate=0.7, tournament_size=3, elite_count=2, tape_length=32, n_examples=64, alphabet=v2_probe, task=sum_gt_10_AND_max_gt_5, disable_early_termination=true, dump_final_population=true, backend=mlx, plasticity_delta=1.0, plasticity_train_fraction=0.75, holdout_size=256, seed_tapes="" (no canonical seeding at sf=0.0). All params byte-identical to §2a/§2c's plastic cells EXCEPT `plasticity_mechanism` (new value `random_sample_threshold`). The `plasticity_budget` spans the same 4 values so per-budget cross-mechanism pairs are matched.
...
- **Est. compute:** per-run wall for `random_sample_threshold` at budget=k is dominated by k train-fitness evaluations per individual (same cost as plastic budget=k but replacing one k-step gradient loop with k independent 48-example eval passes). Projected per-run wall: budget=5 ≈ 7 min, budget=10 ≈ 12 min, budget=20 ≈ 18 min, budget=40 ≈ 30 min (matches plastic timings at same budget). Parallel wall at 10 workers: ~2.5-3h total for 80 new runs. Queue timeout **10800s (3h, 1.5× headroom)** — same as §2c; conservative. Actual per-run cost requires a pilot seed before sweep launches; see Status-transition checklist item 6.
...
**Principle 23 audit:** all 4 cell params byte-identical to §2a/§2c's plastic cells EXCEPT `plasticity_mechanism`. Reusing §2a's budget=5 plastic data (seeds 20..39 subset, n=20) and §2c's budgets {10, 20, 40} plastic data (seeds 20..39 at each, n=20 per budget) requires that sweep YAML's hash-dedup NOT accidentally reuse plastic hashes for random-sample runs (mechanism is hash-relevant since it is NOT at the default when `random_sample_threshold`; `ChemTapeConfig.hash()` pops `plasticity_mechanism` only at its default `rank1_op_threshold`, so any non-default mechanism produces a distinct hash — Status-transition checklist item 3 verifies with explicit pytest).
...
**Baseline quantity 2 — plastic budgets {5, 10, 20} F_AND_test_plastic point estimates + per-cell 98.333% CIs (reused, seeds 20..39 only, n=20 per cell; §26-demoted descriptive-only per codex-v1 P1-1 correction — not routing inputs):**

- Measurement: per-cell seed-bootstrap 98.333% CI on F_AND_test_plastic fraction at each of the three budgets, n=20 per cell. Same bootstrap spec as baseline 1.
- **Values (from §2c chronicle commit `1112e36`; seeds 20..39 subset for all):** budget=5 plastic seeds-20..39: 7/20 = 0.35 (97.5% CI [0.100, 0.600] per §2c); budget=10: 10/20 = 0.50; budget=20: 10/20 = 0.50. §2d's baseline CI routine recomputes at 98.333% for reporting consistency; the point estimates themselves are unchanged.
- Rationale: descriptive per-budget per-mechanism context for chronicle-time discussion. These values do NOT enter any outcome-row conjunction and do NOT affect routing; they are reported at chronicle time as §26-demoted supporting context for the mechanism-interpretation narrative (which itself is chronicle-time §16c work).
...
**Grouping-script attribution (principle 25 clarification):** after the `_cell_key` extension in step 2 above, the per-cell seed-bootstrap CI routine lives in `analyze_plasticity.py:summarize` and the grouping keys cover (arm, plasticity_enabled, plasticity_mechanism, plasticity_budget, seed_fraction) — matching §2d's 4-cell × 2-mechanism grid. The paired-bootstrap routine lives in the NEW function `paired_bootstrap_plastic40_vs_random40` (Status-transition checklist item 1(h)) which takes the per-run CSV + cross-cell extraction as inputs and outputs a CI tuple. No new grouping wrapper script needed; the `summarize` function is the grouping surface.
...
- **Are you running it here?** Yes — random-sample budget=40 at seeds 20..39 is one of the four new cells; the shared-seed paired-bootstrap against reused plastic budget=40 data is the confirmatory test.
...
**Supporting-cell information at budgets {5, 10, 20} — NOT ROUTING (codex-v1 P1-1 correction).** Per §26 demotion, supporting-cell cross-mechanism per-cell point estimates and 98.333% seed-bootstrap CIs at plasticity_budget ∈ {5, 10, 20} are reported at chronicle time as descriptive context. The paired point-estimate signs `F̂_plastic(b) − F̂_random(b)` at these budgets are chronicle-time discussion material only; they DO NOT enter any outcome-row conjunction and DO NOT affect routing. If a sign flips or dramatic cross-mechanism pattern appears at a supporting budget, the chronicle flags this as a §2b grid-miss discovery and the follow-up prereg enumerates it for a future confirmatory. This matches §2c's §26-demoted handling of secondary axes — it does not upgrade the supporting cells into the α budget or the outcome grid.
...
- **Secondary (NOT confirmatory, effect-size only, NO α budget):** per-cell seed-bootstrap 98.333% CI on F_AND_test_plastic fraction at each random-sample budget cell (descriptive visualization); per-cell descriptive statistics on §26-demoted secondary axes; supporting-cell per-cell point estimates and CIs at budgets {5, 10, 20} (reported at chronicle time as descriptive context; NOT in any row-clause conjunction per codex-v1 P1-1 correction).
...
- **Per-sweep test counting (principle 22a):** this prereg produces **ONE** confirmatory test (the paired-bootstrap CI on random-sample budget=40 vs plastic budget=40 at shared seeds 20..39). Supporting-cell per-cell point estimates and CIs at budgets {5, 10, 20} are descriptive-only (§26-demoted, chronicle-time context; do not enter any row-clause conjunction and do not open additional family members — codex-v1 P1-1 correction). Budget=5 is descriptive-only with no routing contribution.
...
**If Row 1 (PASS-POSITIVE — plastic > random at budget=40) fires:** `findings.md#plasticity-narrow-plateau` NARROWS with scope:
`within-task-family · confirmatory cell: n=20 paired at plasticity_budget=40 (plastic rank1_op_threshold vs random_sample_threshold) · descriptive-only (not confirmatory): n=20 per cell at plasticity_budget ∈ {5, 10, 20} same mechanism pair · pop=512 gens=1500 mr=0.03 tournament_size=3 elite_count=2 · sum_gt_10_AND_max_gt_5 natural sampler with 75/25 train/test split · tested mechanisms ∈ {rank1_op_threshold, random_sample_threshold} · δ=1.0 · tested integer budgets ∈ {5, 10, 20, 40} (confirmatory at 40; descriptive at {5, 10, 20}) · sf=0.0 · Arm A only · seeds 20..39`. Mechanism-interpretation (e.g., "plasticity-specific vs other readings") is chronicle-time only under §16c and does NOT appear in this scope tag.

**If Row 3 (SATURATION — CI overlaps 0, non-positive point estimate) fires:** `findings.md#plasticity-narrow-plateau` BROADENS with the same scope tag plus explicit `at tested confirmatory cell plasticity_budget=40 and tested mechanisms ∈ {rank1_op_threshold, random_sample_threshold}, no directional evidence detected between the two mechanisms at matched eval-time compute` qualifier per principle 17b (no extrapolation beyond tested set; no mechanism-name attached to the null direction).
...
- **Principle 17a + 17b:** mechanism axis is single nominal config field (`plasticity_mechanism`) with 3 directly-derived co-moving process variables (δ-selection procedure, reachable-δ space, stopping semantics). All three are direct consequences of mechanism choice — they ARE the mechanism and are named honestly at every qualifier per codex-v1 P2-2 correction (the summary prose and internal-control discussion no longer say "only `plasticity_mechanism` differs"; they say "the mechanism-choice bundle differs, with 3 directly-derived co-moving process variables"). Tested mechanism values ∈ {`rank1_op_threshold`, `random_sample_threshold`} and tested integer budgets ∈ {5, 10, 20, 40} per 17b; confirmatory scope restricts to budget=40 only (supporting budgets descriptive-only); scope qualifiers use verbatim tested-sets.
...
2. **Pre-data: compute baseline CIs on reused plastic data at 98.333% quantile.** Run new CI routines on existing §2a + §2c plastic data (seeds 20..39 at budgets {5, 10, 20, 40}); report point estimate + 98.333% CI per cell. Direct descriptive comparator for H1/H0/H-partial. (~15 min.)
...
4. **Sweep YAML creation.** `experiments/chem_tape/sweeps/v2/v2_5_plasticity_2d.yaml` with 4 new `random_sample_threshold` cells × 20 seeds = 80 runs. Params byte-identical to §2c's plastic cells EXCEPT `plasticity_mechanism: random_sample_threshold`. (~30 min.)

5. **Queue.yaml entry.** Timeout 10800s (3h, 1.5× headroom — matches §2c for 60-to-80-run scale). Reference this prereg path verbatim. (~10 min.)
