...
**Status:** QUEUED (v5, amended) · target commit `{TBD — pin after v5 commit}` · 2026-04-19
...
**Metric definitions (principle 27).** Existing metrics cited from `experiments/chem_tape/analyze_plasticity.py:METRIC_DEFINITIONS` verbatim (same as §1a — see §1a chronicle's METRIC_DEFINITIONS block). Six NEW entries (v5 — `max_gap_at_budget_5`, `max_gap_at_budget_5_cell_boot_ci`, `max_gap_at_budget_5_seed_majority`, `max_gap_at_budget_5_seed_minority_0_05`, `R_fit_delta_paired_sf0`, `initial_population_canonical_count`) are pre-committed verbatim in the "METRIC_DEFINITIONS extensions" block below (before the Status-transition checklist). Row 1's classical-Baldwin exclusion sub-clause additionally reads `Baldwin_gap_h0_mean` and `Baldwin_gap_h1_mean` from the existing `summarize` output — those cell-mean columns are already emitted (no new METRIC_DEFINITIONS entries required). The primary confirmatory statistic — cell-level seed-bootstrap CI + per-seed-majority-0.10 count + per-seed-minority-0.05 count on `max_gap_at_budget_5` + the row-1 h=0/1 cell-mean sub-clause — is NOT fully emitted by `analyze_plasticity.py` yet; Status-transition checklist item 1 covers the extensions.
...
|---|---------|-------------------|--------------|--------------------|--------------------------|
| 1 | **F-RECOVERY-WITHOUT-INVERSE-SIGNATURE** (v3 renamed from BALDWIN-EMERGES) | ≥ 0.40 (40-pp lift) AND frozen ≤ 0.45 (v5 asymmetric cap) | any | cell-bootstrap CI_hi < 0.05 AND fewer than 10/20 seeds have per-seed max_gap > 0.05 (tail-gap absent; INVERSE-BALDWIN signature does NOT survive shortcut removal) AND `max(Baldwin_gap_h0_mean, Baldwin_gap_h1_mean) < 0.05` (v5 — classical-Baldwin exclusion; non-zero h=0/1 cell-mean gap routes to row 7) | Plasticity recovers canonical-equivalents without the static shortcut; selection-deception diagnosis **remains viable / consistent with** the shortcut-removal-unlocks-F-recovery signal (v5 softening). Baldwin-direction NOT established — per-seed Hamming analysis of top-1 winners required at chronicle time to confirm. Next leg: §v2.5-plasticity-2b (EES). Rank-2 deferred. Findings.md `plasticity-narrow-plateau` narrows. |
| 2 | **UNIVERSAL-ADAPTER** | ≥ 0.40 AND frozen ≤ 0.45 (v5 asymmetric cap) | ≤ 1.5 | any | Plastic discovery from noise via convergent δ. Selection-deception weakly supported; mechanism is "δ does the work regardless of genotype." EES candidate. Findings.md narrows. |
...
**Row-clause fidelity (principle 28a pre-commitment).** Row 3 requires ALL three sub-clauses simultaneously: (a) `(plastic_F_prop − frozen_F_prop) ≤ 0.15`; (b) δ_std at budget=5 > 2.0; (c) `max_gap_at_budget_5` cell-bootstrap CI_lo ≥ 0.10 AND ≥ 10/20 seeds have per-seed max_gap > 0.10. Prose-match with any sub-clause failing = row 7 (grid-miss), NOT a row-3 match. Row 5 (universal-null) requires its own 4-way conjunction: F-lift ≤ 0.15 AND `frozen_F_prop ≤ 0.45` AND δ_std ≤ 1.5 AND CI_hi < 0.05 AND `max_gap_at_budget_5_seed_minority_0_05 < 10` (fewer than 10/20 seeds per-seed max_gap > 0.05). Row 1 (v5 — four-way conjunction on confirmatory axis) requires F-lift ≥ 0.40 AND `frozen_F_prop ≤ 0.45` AND gap-absence (CI_hi AND per-seed-minority-0.05) AND classical-Baldwin exclusion (`max(Baldwin_gap_h0_mean, Baldwin_gap_h1_mean) < 0.05`). Row 2 requires F-lift ≥ 0.40 AND `frozen_F_prop ≤ 0.45` AND δ_std collapse. Row 6 (SWAMPED) fires ONLY on the high-side cap `frozen_F_prop > 0.45` per v5.
...
- **Frozen plausibility cap `frozen_F_prop ≤ 0.45` (row 6 SWAMPED boundary — v5 asymmetric per codex-v4 P1-a).** §1a drift frozen F = 3/20 = 0.15 is the anchor. Binomial(20, 0.15) low-side: P(F=0) ≈ 3.9%, P(F=1) ≈ 13.7%, P(F ≤ 1) ≈ 17.6% — a symmetric low-side cap like v4's 0.10 would misfire SWAMPED on ~17% of noise-only runs, which is ordinary sampling noise, not infrastructure bug. High-side: P(F ≥ 9) ≈ 0.002 under the anchor, so `frozen_F_prop > 0.45` is genuine evidence of canonical leakage or trivial-task behavior. v5 row 6 therefore fires ONLY on the high-side cap. Low-side oddities (F=0 or F=1 seeds) are covered by the `initial_population_canonical_count > 0` pytest invariant + chronicle-time manual inspection (see Infrastructure-fidelity check block above); they are NOT a row-6 trigger.
...
- **Principle 25:** `max_gap_at_budget_5` cell-level seed-bootstrap CI + per-seed-majority + per-seed-minority-0.05 (v5) + row-1 classical-Baldwin-exclusion sub-clause on `Baldwin_gap_h0/h1` cell-means is the required confirmatory statistic package; frozen-only run processing + cross-cell merge for `R_fit_delta_paired_sf0` is the secondary diagnostic. Most are not currently emitted by `analyze_plasticity.py` — six new METRIC_DEFINITIONS entries pre-committed verbatim in the Metric-definitions-extensions block (v5 adds `max_gap_at_budget_5_seed_minority_0_05` to v4's five); Status-transition checklist item 1 covers the code. The `Baldwin_gap_h0_mean` and `Baldwin_gap_h1_mean` cell-mean columns already exist in `summarize` output — no additional engineering for the row-1 classical-Baldwin exclusion sub-clause.
...
"max_gap_at_budget_5_seed_minority_0_05": (
    "Count of seeds (out of 20 in the cell) with per-seed "
    "max_gap_at_budget_5 > 0.05. NaN seeds (sparse-bin guard) are "
    "excluded from the count denominator — count is over non-nan seeds, "
    "threshold for row firing is count < 10. Part of §v2.5-plasticity-2a's "
    "row-1 and row-5 tail-absence criterion: cell-bootstrap CI_hi < 0.05 "
    "AND this count < 10. Symmetric upper-bound counterpart to "
    ...
    "left the 0.05 minority count undefined."
),
...
1. **`analyze_plasticity.py` extensions (~105 min total; v3 under-counted CSV schema normalization per codex-v3; v5 adds ~5 min for item 1(g) seed-minority-0.05 count per codex-v4 P2-a):**
   ...
   - (f) Add the 6 METRIC_DEFINITIONS entries verbatim from the block above (v5 — one more than v4's 5, for `_seed_minority_0_05`). ~5 min.
   - (g) **NEW in v5 — UNDISCHARGED at v5 commit time.** Add `max_gap_at_budget_5_seed_minority_0_05` count in `summarize` (count of seeds with per-seed max_gap > 0.05, excluding nan). Rows 1 and 5 trigger on this count < 10. Exactly parallel to (d)'s existing majority branch — same pattern, different threshold. ~5 min. Must land before QUEUED → RUNNING transition.
...
7. **Codex adversarial review of v5** — focused on (a) whether the asymmetric `frozen_F_prop > 0.45` SWAMPED cap is well-calibrated under the §1a anchor and correctly leaves low-side noise for the pytest invariant; (b) whether the row-1 classical-Baldwin exclusion sub-clause (`max(Baldwin_gap_h0_mean, Baldwin_gap_h1_mean) < 0.05`) cleanly distinguishes shortcut-removal-unlocks-F-recovery from classical Baldwin; (c) whether the `max_gap_at_budget_5_seed_minority_0_05` METRIC_DEFINITIONS entry is consistent with rows 1/5 and with the existing `_seed_majority` counterpart; (d) whether the softened "remains viable / consistent with" prose for row 1 is appropriately calibrated (not over-softened to vacuity). v4 returned FAIL on this gate with 4 P1 + 2 P2 findings (resolved per the v4 → v5 amendment block above); v5 must pass before sweep launch.

**Total engineering effort:** ≈ 2.5-3 h (v3's estimate was ~2-2.5 h; v4 added CSV schema normalization per codex-v3; v5 adds ~5 min for the seed-minority-0.05 count per codex-v4 P2-a). Both v2 and v3 under-counted; v4 was the honest full scope plus one new ~5-min analyzer field in v5.
...
- Prior commits: `9ff9bf8` — v1; `f6ead25` — v2 amendment; `c8cf17b` — v3 amendment; `09ceebd` — v4 amendment (SHA-pinned at `aef841e`); all superseded by this v5; reasoning trail preserved in git history per §13 spirit.
