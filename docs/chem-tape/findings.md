...
**Status:** `ACTIVE` · last revised commit TBD (this session) · 2026-04-18 (mechanism reading narrowed to **monotone single-mechanism cloud-destabilisation under BP_TOPK preserve at selection pressure ≥ tournament_size=3** via §v2.4-proxy-5d v1 replication + §v2.4-proxy-5c-tournament-size pressure probe). Kinetic qualifier from §v2.4-proxy-5b-amended remains **added on exploratory evidence** — confirmatory cross-task test `§v2.4-proxy-5b-crosstask` + budget-rate decoupling cell both still required before paper-level citation. Provisional kinetic reading: both decoder arms have a mutation-rate variation-layer lever — Arm A 225× R_fit lift, BP_TOPK 1.31× R_fit lift at mr=0.005; decoder-specific mechanism split retained: BP_TOPK = canonical off-center in wide solver neutral network with kinetically-modulated within-cloud distribution; Arm A = classical proxy-basin population dynamics with kinetically-scaled erosion.
...
**Selection-pressure floor qualifier (ADDED 2026-04-18).** §v2.4-proxy-5c-tournament-size probed tournament_size ∈ {2, 3, 5, 8}. Result: cliff+plateau shape — R_fit_999 ≈ 0.005 at ts=2 (solvers found at best-of-run but don't propagate; weak-selection starvation regime) vs {0.723, 0.740, 0.746} at ts ∈ {3, 5, 8} (plateau within ±0.023). The wide solver neutral network is decoder-intrinsic at working selection pressures (ts ≥ 3) and requires some minimum pressure to form. The `ts ≥ 3` qualifier is a necessary condition, not sufficient; the mechanism itself is the BP_TOPK decoder's many-to-one mapping producing a wide off-center solver cloud.
...
- 2026-04-18 (later same day) — **non-monotone subsidiary reading
  ...
  plateau at ts ∈ {3, 5, 8}. The `at selection pressure ≥ ts=3`
  qualifier is added to the mechanism name as a necessary condition.
