...
Corrected α = 0.05/3 ≈ 0.017; all three p-values clear this by > 4 orders of magnitude. **Pre-§22 tests (§v2.4 compute-scaling, §v2.4-proxy decorrelation, §v2.4-proxy-2, §v2.4-proxy-3, §v2.12 Arm A, §v2.14b) are descriptive-exploratory supporting evidence, NOT confirmatory family members** — they have no post-§22 classification and their McNemar statistics (where computed) are reported in-chronicle for reasoning-trail purposes, not for FWER gating. The pending `§v2.4-proxy-5d-followup-cloud-reexpansion` prereg, if promoted, would grow the family to 4 with α = 0.05/4 = 0.0125; all existing tests clear this tighter α by > 3 orders of magnitude. See `Plans/fwer_audit_2026-04-18.md` for the full audit (supersedes `fwer_audit_2026-04-17.md`, which under-counted F1 by 2 tests).
