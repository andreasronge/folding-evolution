...
## §v2.5-plasticity-2a-nexp-budget5. Pre-committed row-4 n-expansion (seeds 40..59 at Arm A budget=5 plastic) — **Row 4 AMBIGUOUS/PARTIAL persists on pooled n=40; F-lift = 0.35 identical across halves; mechanism-axis signal TIGHTENS (max_gap_at_budget_5 CI [+0.267, +0.346] on pooled vs §2a's [+0.196, +0.311]; seed_majority 37/40; seed_minority_0_05 37/40); §28a prose-fit × F-lift-clause-fail on row 3 intensifies — §2b follow-up prereg required per §2a chronicle pre-commitment** (2026-04-22)

**Status:** `INCONCLUSIVE — matched pre-registered AMBIGUOUS/PARTIAL row (row 4 clean clause-match on pooled n=40; status token INCONCLUSIVE per §log-result standardized vocabulary); pooled F-lift = 0.35 identical to §2a's 0.35 with identical per-half F_plastic counts (§2a=7/20, n-exp=7/20, pooled=14/40); mechanism-axis signal TIGHTENS on pooled n=40 (max_gap_at_budget_5 CI [+0.267, +0.346] vs §2a's [+0.196, +0.311]; seed_majority 37/40; seed_minority_0.05 37/40; δ_std=2.590); row 3 §28a prose-fit × F-lift-clause-fail pattern persists and intensifies — 4/5 clauses satisfied with tighter CI (again NOT partial row-3 evidence per §28a near-match rule); §2b follow-up prereg required per §v2.5-plasticity-2a chronicle Next-steps bullet 5 (case ii resolution: row 4 persists in mid-range window); EES (§v2.5-plasticity-2b) and rank-2 (§v2.5-plasticity-1b) REMAIN NOT queued; findings.md still NOT amended` · n=20 n-exp + n=20 §2a pooled = n=40 at budget=5 plastic · commit `9ad15ea` · continues §v2.5-plasticity-2a

**Pre-reg:** [Plans/prereg_v2-5-plasticity-2a.md](../../Plans/prereg_v2-5-plasticity-2a.md) v14-closure, **row-4 decision rule at line 444** (pre-committed n-expansion for the AMBIGUOUS/PARTIAL outcome). This chronicle is a pre-committed continuation of §v2.5-plasticity-2a; no new prereg required per §2a chronicle Next-steps bullet 1 ("**Does NOT require a new prereg** if interpretation is against the existing rows 1–5 (prereg explicitly pre-committed this extension)").
**Sweep:** `experiments/chem_tape/sweeps/v2/v2_5_plasticity_2a_nexp_budget5.yaml` (committed 9ad15ea)
**Compute:** 34m59s wall · 10 workers · 20 runs (projected ≈ 20-25 min; actual longer due to per-run budget=5 plastic execution variance in the 7-10 min/run range per §2a's 94m49s / 100 runs ≈ 9.5 min/run average)

### Question

Per §v2.5-plasticity-2a chronicle's row-4 decision rule: does n-expansion on seeds 40..59 at Arm A budget=5 plastic resolve the F-lift ambiguity (0.35 at n=20, in the pre-committed row-4 mid-range window (0.15, 0.40))?

### Hypothesis (pre-registered in the §v2.5-plasticity-2a chronicle's Interpretation (2))

Three possible resolutions were pre-committed in the §2a chronicle (commit 2fedf7d, Interpretation (2)):
- (i) F-lift drops to ≤ 0.15 on pooled n=40 → row 3 fires cleanly on all five clauses → P-1 falsification confirmed, rank-2 queued ahead of EES.
- (ii) F-lift stays in (0.15, 0.40) → row 4 persists, §2b-enumerated follow-up prereg is what routes.
- (iii) F-lift rises to ≥ 0.40 → row 1's F-lift clause would satisfy but row 1's `max_gap` clause is already failed by a wide margin and will not pass on n-expansion absent a large distributional shift — also routes to a §2b-enumerated row on the follow-up prereg.

Only case (i) had a clean pre-registered next-step; cases (ii) and (iii) both require the follow-up prereg's §2b enumeration.

### Result

**Per-half + pooled table (budget=5 plastic only; frozen control NOT re-expanded per prereg row-4 "~20 min wall" pre-commitment):**

| slice | n | F_AND_test_plastic (`best_fitness_test_plastic ≥ 1.0`) | F_AND_train_plastic | `max_gap_at_budget_5` mean [CI_lo, CI_hi] | seed_maj(>0.10) | seed_min_0.05(>0.05) | non-nan | δ_std (`delta_final_std_mean`) | R_fit_plastic_999 | GT_bypass | top1_winner_hamming |
|-------|---|-------------------------------------------------------|--------------------|-------------------------------------------|-----------------|----------------------|---------|--------------------------------|-------------------|-----------|---------------------|
| §2a seeds 20..39 (prior) | 20 | 7/20 | 2/20 | 0.2546 [+0.196, +0.311] | 17/20 | 17/20 | 20/20 | 2.714 | 0.0584 | 0.007 | all 20 at 5 |
| §2a-nexp seeds 40..59 (new) | 20 | 7/20 | 2/20 | 0.3596 [+0.316, +0.403] | 20/20 | 20/20 | 20/20 | 2.466 | 0.0654 | 0.006 | all 20 at 5 |
| **Pooled n=40** | **40** | **14/40 = 0.35** | **4/40 = 0.10** | **0.3071 [+0.267, +0.346]** | **37/40** | **37/40** | **40/40** | **2.590** | **0.0619** | **0.007** | **all 40 at 5** |
| §2a frozen control (reused) | 20 | — | — | — | — | — | — | — | 0.000 | — | — |

Frozen control cell: `best_fitness ≥ 1.0` count = **0/20** (reused from §2a; NOT re-expanded per prereg row-4 "~20 min wall" pre-commitment). **F-lift = pooled_plastic_F_prop − frozen_F_prop = 14/40 − 0/20 = 0.35 − 0.0 = 0.35** (identical to §2a's 0.35; two identical observed per-half plastic counts of 7/20 anchor the result).

**Bootstrap CI methodology:** pooled n=40 CI computed via seed-bootstrap (10 000 resamples, `numpy.random.default_rng(seed=42)`, 95% quantiles of resampled means) — matches `bootstrap_ci_spec` from `analyze_plasticity.py` METRIC_DEFINITIONS verbatim.

**Matches pre-registered outcome:** **Row 4 (AMBIGUOUS / PARTIAL) persists on pooled n=40** — clean clause-match on all three row-4 numeric clauses (§28a):

- (a) `0.15 < (plastic_F_prop − frozen_F_prop) < 0.40`: observed **0.35** ✓ (identical to §2a's 0.35; within row-4 window)
- (b) `frozen_F_prop ≤ 0.45`: observed **0.0** ✓
- (c) `20/20 non-nan seeds on max_gap_at_budget_5` → scaled to **40/40 non-nan** at n=40 ✓ (the prereg literal "20/20" is the 20-seed-cell denominator; the pooled 40-seed cell's equivalent hard floor is 40/40 non-nan, which the data satisfy)

Status-vocabulary mapping (§log-result): the matched prereg row's name is **AMBIGUOUS / PARTIAL** (pre-committed as the mid-range F-lift outcome); the standardized status token is **INCONCLUSIVE**. These are distinct — the matched row carries a specific pre-committed decision rule (n-expansion was itself that rule, now discharged; the §2b follow-up prereg commitment takes over), whereas "INCONCLUSIVE" alone is a vocabulary token that covers multiple routing dispositions.

**Row exclusion audit at n=40 (all rows explicitly re-checked against pooled data):**

| row | clauses satisfied | clauses failed | matches? |
|-----|-------------------|----------------|----------|
| 1 (F-RECOVERY-WITHOUT-INVERSE-SIGNATURE) | (b) frozen ≤ 0.45; (d) 40/40 non-nan | (a) lift ≥ 0.40 → 0.35 fails; (c) CI_hi < 0.05 → 0.346 fails AND seed_min_0.05 < 10 → 37 fails **by a wider margin than §2a's n=20** | NO |
| 2 (UNIVERSAL-ADAPTER) | (b); (d) | (a) 0.35 < 0.40 fails; (c) δ_std ≤ 1.5 → 2.590 fails | NO |
| 3 (INVERSE-BALDWIN-REPLICATES) | (b); (c) δ_std > 2.0 → 2.590 ✓; (d) CI_lo ≥ 0.10 → **0.267** ✓ (tighter than §2a's 0.196) AND seed_maj ≥ 10 → **37** ✓ (up from §2a's 17); (e) 40/40 non-nan | **(a) lift ≤ 0.15 → 0.35 fails** (identical to §2a; n-expansion did NOT resolve toward row-3 territory) | **NO — §28a prose-fit / clause-fail on F-lift clause alone; 4/5 clauses satisfied on mechanism axes with TIGHTER CI than §2a — the near-match structure intensifies but does NOT carry row-match force under §28a** |
| **4 (AMBIGUOUS / PARTIAL)** | **(a) 0.15 < 0.35 < 0.40 ✓ (replicated); (b) 0.0 ≤ 0.45 ✓; (c) 40/40 non-nan ✓** | **—** | **YES — clean clause-match at n=40 (identical F-lift to §2a)** |
| 5 (FAIL — universal-null) | (b) | (a) 0.35 ≰ 0.15 fails; (c) δ_std ≰ 1.5 fails; (d) CI_hi < 0.05 fails AND seed_min_0.05 < 10 fails | NO |
| 6 (SWAMPED) | — | frozen=0.0, row 6's `> 0.45` fails | NO |
| 7 (INCONCLUSIVE — grid-miss catchall) | — | row 4 matches cleanly (row 7 trigger (c) does not apply); 40/40 non-nan (trigger (b) does not apply); trigger (a) is a §28a prose-fit × F-lift-clause-fail observation on row 3 (now intensified on tighter CI) but does not override a clean row-4 match | NO |

Row 3 continues to be a §28a prose-fit / clause-fail (mechanism axes match row-3 prose at a tighter CI; row-3's F-lift clause fails at identical 0.35). Per §28a this is a §2b grid-miss on row 3, not a row-3 match. Four-of-five clauses satisfied on mechanism axes **does NOT** carry evidential weight toward row 3 (§28a near-match has no row-match force); the audit is reported only to document that n-expansion intensifies the §2b prereg-design gap identified in §v2.5-plasticity-2a's Interpretation (2).

**Statistical test** (primary confirmatory, three-clause conjunction on `max_gap_at_budget_5` at budget=5, evaluated on pooled n=40):

- (i) cell-level seed-bootstrap 95% CI = **[+0.2665, +0.3457]** (tighter than §2a's [+0.1959, +0.3109] as expected under doubled n; still excludes 0 on the positive side; would satisfy row 3's CI_lo ≥ 0.10 clause).
- (ii) `max_gap_at_budget_5_seed_majority` (count > 0.10) = **37/40** (satisfies row 3's ≥ 10-on-20 clause → ≥ 20-on-40 equivalent by wide margin; per-seed signal is strongly positive across the pooled block).
- (iii) `max_gap_at_budget_5_seed_minority_0_05` (count > 0.05) = **37/40** (fails row 1's < 10-on-20 clause → < 20-on-40 equivalent by wide margin; far from row-1's tail-absence requirement).

**Classification:** confirmatory. **Family:** `plasticity-inverse-baldwin-replicates`, size 1, corrected α = 0.05/1 = 0.05 (unchanged from §2a per §22b commit-time-membership — the family was opened at §2a-chronicle-time and remains size 1; n-expansion is a pre-committed intra-cell refinement, not a new test). **Per-sweep count:** 1 (the three-clause conjunction evaluated together on the pooled cell; it is NOT three separate family members per §22a, matching the §2a chronicle's explicit per-sweep counting). Per principle 22b commit-time-membership, the confirmatory test ran and consumed α budget regardless of row-4 (ambiguous) resolution; family size remains 1 and the null will be recorded under §24 when the family eventually closes (the row-4 persistence does not close the family — the §2b follow-up prereg's row may replace this test or extend it).

### Pre-registration fidelity checklist (required, principle 23)

- [x] **Every outcome row in the prereg was tested.** Rows 1–7 all re-evaluated at pooled n=40 (see row exclusion audit table above). Row 4 fired cleanly; rows 1, 2, 3, 5 failed on ≥ 1 numeric clause; row 6 failed on direction; row 7 catchall not triggered. No silent addition or removal of rows. The n-expansion pre-commitment (§2a prereg row-4 decision rule) is discharged.
- [x] **Every part of the pre-committed n-expansion plan ran to completion.** Sweep YAML `experiments/chem_tape/sweeps/v2/v2_5_plasticity_2a_nexp_budget5.yaml` (committed 9ad15ea) ran 20/20 seeds 40..59 at Arm A budget=5 plastic; no re-running of budgets 1, 2, 3 (exploratory, not pre-committed for expansion); no re-running of frozen control (prereg pre-commitment: frozen not re-expanded). Process-fidelity note documented in Caveats below: an initial launch attempted at 13:24 had a Bash-variable-expansion failure that caused workers to resolve `--output-root ""` → `Path(".")` and write to the repo root; workers were killed via pkill at 13:35, stray empty directories cleaned, and the sweep relaunched at 13:41 with a literal absolute `--output-root` path; the relaunched workers completed at 14:02 with hash-dedup cache hit on cleanup-deleted-then-relaunched runs producing a small number of zombie worker writes to the repo root (content checksum-verified identical to the correct-dir output; strays deleted). Data integrity: all 20 runs' `history.npz` checksums in `experiments/output/2026-04-22/v2_5_plasticity_2a_nexp_budget5/` are authoritative; no mid-run parameter changes.
- [x] **No parameters, sampler settings, or seed blocks were changed mid-run.** Sweep YAML byte-identical to the queued version. Seeds 40..59 on the budget=5 plastic cell per pre-commitment. Fixed params match §2a verbatim (pop=512, gens=1500, mr=0.03, tournament_size=3, elite_count=2, plasticity_mechanism=rank1_op_threshold, plasticity_delta=1.0, plasticity_train_fraction=0.75, tape_length=32, n_examples=64, holdout_size=256, alphabet=v2_probe, task=sum_gt_10_AND_max_gt_5, seed_tapes="" at sf=0.0, backend=mlx).
- [x] **Every statistical test and diagnostic named in the prereg appears.** Confirmatory 3-clause `max_gap_at_budget_5` statistic: reported on pooled n=40 above. Diagnostics: all items from the §2a prereg's "Diagnostics to log" block are reported below in the Diagnostics (prereg-promise ledger) block; for items only applicable at budgets 1/2/3 (not re-run), the §2a values still stand and are cross-referenced. Infrastructure-fidelity check (`initial_population_canonical_count`): 0/20 on the n-exp runs — PASS.

### Interpretation

**(1) Row 4 (AMBIGUOUS / PARTIAL) persists on pooled n=40; F-lift = 0.35 is replicated exactly across halves; chronicle status token is INCONCLUSIVE per §log-result vocabulary.** The n-expansion resolved to **case (ii) from the §v2.5-plasticity-2a chronicle's Interpretation (2) pre-commitment**: F-lift stays in (0.15, 0.40) and row 4 persists. The per-half F_AND_test_plastic counts are identical (§2a: 7/20, n-exp: 7/20), giving pooled 14/40 = 0.35 — the same observed count and same lift value at n=40 as at n=20. Per §2a chronicle's §2b follow-up commitment (Next-steps bullet 5), this outcome **requires a follow-up prereg that enumerates the (row-3-mechanism-signature × row-4-F-lift) cell as a dedicated outcome row before interpreting**. Today the prereg's routing rule for row 4 is fully discharged; EES (§v2.5-plasticity-2b) and rank-2 (§v2.5-plasticity-1b) REMAIN NOT queued because neither row 1 nor row 3 fired. Parallel rank-2 engineering (VM implementation, 3–5 days) MAY continue during the §2b prereg-amendment cycle but remains a separate engineering call this chronicle does not pre-commit. The matched prereg row name (AMBIGUOUS / PARTIAL) and the status-vocabulary token (INCONCLUSIVE) remain distinct per §log-result vocabulary.

**(2) Mechanism-axis signal INTENSIFIES on pooled n=40; row 3's §28a prose-fit × F-lift-clause-fail structure tightens (this is NOT partial row-3 evidence).** The pooled cell's `max_gap_at_budget_5` numbers are all stronger than §2a's: CI [+0.267, +0.346] (vs §2a [+0.196, +0.311], a tighter 95% interval further from 0); seed_majority 37/40 (vs 17/20); seed_minority_0_05 37/40 (vs 17/20). δ_std = 2.590 (pooled) remains above row 3's > 2.0 threshold (between §2a's 2.714 and n-exp's 2.466; the drop in n-exp is within sampling variance). Four-of-five row-3 clauses are now satisfied on a tighter CI than §2a (a+b+c+d+e excluding F-lift clause). **Per §28a, this is emphatically NOT partial row-3 evidence**: near-match has no row-match force; row 3 requires all five clauses conjunctively. The intensification documents that the §2b prereg-design gap identified at §2a-chronicle-time (row 4's "any" for max_gap/δ_std was under-scoped on a diagnostic axis that now carries consistent row-3-level signal across disjoint seed blocks) is real and replicates; the follow-up prereg's §2b enumeration becomes more important, not less. The prereg's pre-committed decision rule (row 4 → n-expansion → §2b follow-up on persistence) is followed verbatim.

**(3) The replicated plastic F count and frozen-baseline-noise caveat on the F-lift comparison.** The observed plastic F count at budget=5 is **7/20 on both disjoint seed blocks** (§2a seeds 20..39 and n-exp seeds 40..59), giving pooled 14/40 = 0.35. This is consistent with a replicated discovery rate for rank-1 plasticity at sf=0.0 budget=5 across the two seed blocks; same observed count on two n=20 draws does not by itself establish strict distributional equivalence, so the anti-drift point here is tied to the matched counts, not a stronger "identical distribution" reading. The frozen-baseline comparison is anchored to the existing §2a frozen control cell (0/20 on seeds 20..39, NOT re-expanded per prereg pre-commitment); the pooled plastic(40) − frozen(20) lift is 0.35, identical to §2a's 0.35 because both halves contributed the same plastic F count. The earlier §2a chronicle's note that the 0.20 → 0.35 apparent F-lift shift between §1a drift and §2a is plausibly consistent with frozen-baseline Binomial(20, p) sampling noise still stands at this pooled reading — the §2a vs §2a-nexp comparison has NO frozen-baseline shift by construction (same control) and produces a stable F-lift of 0.35.

**(4) `top1_winner_hamming` = 5 on all 40/40 winners (cap+1 sentinel) is a strong sentinel-pattern — flagged as an explicit inspection obligation for the §2b follow-up prereg, NOT folded into the current mechanism read; does NOT rescue row 3.** Across all 40 plastic runs at budget=5 (both halves), every top-1 winner has `top1_winner_hamming = 5`. This is NOT pure noise — 40/40 at the same sentinel value across two disjoint seed blocks (§2a seeds 20..39 and n-exp seeds 40..59, under otherwise-identical config) indicates that **something structured is persisting about the winners' active-view structure at this sweep configuration**: every winning tape lands at Levenshtein-active-view distance ≥ 5 from the canonical `sum_gt_10_AND_max_gt_5` body, without exception. This consistent-far-from-canonical-active-view signature is diagnostic-level evidence (consistent with row 3's INVERSE-BALDWIN-REPLICATES prose, which anticipates "distant genotypes solve under plasticity without approaching canonical") **but it does NOT rescue row 3 per §28a** — row 3 still requires F-lift ≤ 0.15, which fails at 0.35. The sentinel-consistency is reported here as a chronicle-time diagnostic, not as partial row-3 evidence. **Inspection obligation pre-committed for the §2b follow-up prereg:** the follow-up prereg must (a) inspect the 40 winner tapes directly to classify their attractor structure at the active-view level (what ARE they solving with, if not near-canonical?), (b) re-evaluate whether the sentinel at cap=4 is limiting information (cap=8 or uncapped inspection may reveal further structure in the 5..N range), (c) include this 40/40-sentinel pattern as a scope-dimension in its outcome-grid enumeration. Under the §v2.5-plasticity-2a prereg's chronicle-time CB classification (≤ 1 = CB ACTIVE, ≥ 2 = CB INACTIVE), all 40 would classify as CB INACTIVE (`n_classical_active = 0`, `n_classical_inactive = 40`, routing = CB NON-DOMINANT if row 1 had fired). Row 1 did not fire, so the CB block is skipped per the prereg's "When this block fires" clause (line 408). **The §25/§27 description-vs-implementation drift flagged in the §v2.5-plasticity-2a chronicle persists and replicates**: the prereg and analyzer METRIC_DEFINITIONS entries state `top1_winner_hamming ∈ {0, 1, 2, 3, 4}` while the `levenshtein(a, b, cap=4)` implementation at `analyze_retention.py:177-206` returns `cap + 1 = 5` as a sentinel for "distance exceeds cap"; the observed range is {5} on both halves. The follow-up prereg's cap-inspection above is the natural place to discharge this — either correcting the definition to ∈ {0, 1, 2, 3, 4, 5} with cap=4 preserved, or raising the cap. Acknowledged as methodology-backlog item from §2a; this chronicle does not re-discharge it (same drift, same disposition).

### Chronicle-time classical-Baldwin disambiguation discipline (§2a prereg §25b option c pre-commitment)

**Did not fire** (same as §2a). Per prereg line 408, the CB block fires only when row 1 fires. Row 1 did not fire on pooled n=40 (F-lift clause `≥ 0.40` and `max_gap_at_budget_5` tail-absence clause both fail by wider margins than at n=20). Artifact-complete floor: all 40 runs (§2a + n-exp) pass — every `final_population.npz` loads with the required schema (`genotypes`, `test_fitness_plastic`, `train_fitness_plastic` present, consistent length 512); zero BLOCKED seeds.

For diagnostic transparency: `n_cb_active = 0`, `n_cb_inactive = 40` on pooled data (reported in Result table). The CB verdict would have been **CB NON-DOMINANT** (`n_classical_active ≤ 9-equivalent-on-20, or ≤ 18-equivalent-on-40`) IF row 1 had fired; since row 1 did not fire, this is diagnostic only and does NOT modify the row-4 routing.

### Caveats

- **Seed count:** n=40 pooled at the confirmatory budget=5 cell (seeds 20..59, disjoint from §1a's 0..19). Row-4 decision rule pre-committed the n-expansion; this chronicle discharges that commitment. Further n-expansion (beyond n=40) is NOT pre-committed by any current prereg — the §2b follow-up prereg would need to enumerate whether further expansion is appropriate.
- **Frozen control was NOT re-expanded.** Per §v2.5-plasticity-2a prereg row-4 "~20 min wall" pre-commitment, the n-expansion scope was plastic-only at budget=5. The pooled n=40 plastic cell is compared against the n=20 frozen control cell from §2a (seeds 20..39). This asymmetric pooling is explicit per the prereg; the §2b follow-up prereg should state whether frozen-control extension is needed for its own routing logic.
- **Compute:** 34m59s wall vs 3600s (1h) queue timeout gave ×1.7 headroom; not re-tuning needed unless further expansion is desired.
- **Launch process-fidelity note (principle 23 honest disclosure).** The sweep was launched twice: first at 13:24 with a Bash-heredoc `$RUN_DIR` env-var-expansion failure that caused sweep.py workers to resolve `--output-root ""` → `Path(".")` and write to the repo root; the broken run was killed via pkill at 13:35 and the 10 stray directories were cleaned. The second launch at 13:41 used a literal absolute `--output-root` path and produced the authoritative results in `experiments/output/2026-04-22/v2_5_plasticity_2a_nexp_budget5/`. A small number of zombie worker writes from the first launch surfaced in the repo root during the second launch's window (timestamps 13:44-13:48 on ~10 hash dirs); content-checksum verification confirmed those strays were identical copies of the correct-dir output, and the strays were deleted after verification. **Only the `experiments/output/2026-04-22/v2_5_plasticity_2a_nexp_budget5/` directory is authoritative for the reported numbers** — the `shasum` verification above confirmed equality with the zombie strays before deletion, but the chronicle's statistics, CIs, guard discharges, and every downstream number derive exclusively from the correct-dir 20 runs (combined with the §2a cell's 20 runs in `experiments/output/2026-04-21/v2_5_plasticity_2a/`). No chronicle number was read from a repo-root stray.
- **Overreach check (principle 17).** Scope-qualified: all claims are at `within-task-family · n=40 pooled at budget=5 plastic (20 from §2a + 20 from §2a-nexp, disjoint seeds 20..59) · n=20 frozen control (not re-expanded) · pop=512 gens=1500 mr=0.03 tournament_size=3 elite_count=2 · sum_gt_10_AND_max_gt_5 natural sampler with 75/25 train/test split · rank1_op_threshold mechanism · δ=1.0 · tested integer budget values ∈ {1, 2, 3, 5} (budgets 1/2/3 data from §2a only, not re-expanded) · random initial population sf=0.0 · Arm A only (BP_TOPK EXCLUDED per prereg's structural R_fit ceiling caveat)`. Principle 17b: "at budgets ∈ {1, 2, 3, 5}" not "at budget ≤ 5".
- **Open mechanism questions (unchanged from §2a):** (a) whether the §2b follow-up prereg's dedicated (row-3-mechanism-signature × row-4-F-lift) row fires as a genuine new outcome category or reveals further subcategories; (b) whether rank-2 memory depth breaks the mechanism-axis / F-lift decoupling if the §2b prereg routes there; (c) cross-task scope (P-3 from §v2.5-plasticity-1a's Falsifiability block — untested); (d) whether EES or novelty-search exposes different dynamics in the selection-deception branch (currently unreachable cleanly: the pre-committed EES route requires row 1 to fire, which requires max_gap's tail-absence clause that this sweep rules out at the tested cell with a tighter CI than §2a).

### Degenerate-success check (principle 4 — six guards re-evaluated on pooled n=40 where applicable)

Guards enumerated per §v2.5-plasticity-2a prereg §Degenerate-success-guard (lines 360-369). n-exp contributes 20 additional runs at budget=5 plastic; guards 1, 2, 3, 4, 5 are re-evaluated on pooled data, guard 6 is reproduced on n-exp.

1. **Guard 1 — Universal-adapter artefact (row 2):** NOT TRIGGERED. Top-1 winner h=0 rate = 0/40 (all 40 `top1_winner_hamming = 5`). Not a universal-canonical-recovery mechanism.
2. **Guard 2 — Train-test leakage:** NOT TRIGGERED. Pooled at budget=5: F_AND_test_plastic − F_AND_train_plastic = 14/40 − 4/40 = **+10/40 = +0.25** (test solve rate exceeds train — identical to §2a's +0.25 direction; each half contributed +5/20). Not near-zero; discovery rate is 14/40 = 0.35, not high. Guard discharged.
3. **Guard 3 — Threshold-saturation artefact at budget=5 (population + top-1 winner split):** OBSERVED on n-exp. n-exp alone: pop-level `|δ_final| ≥ 5` fraction = **0.8614** across 20 seeds (vs §2a's 0.7115); top-1 winner `|δ_final| ≥ 5` = **17/20** (vs §2a's 15/20). Pooled top-1: **32/40**. N-exp shows stronger saturation than §2a, consistent with §1a drift's 0.738 pop / 14/20 top-1 precedent. Per prereg guard-3 text: "saturated mechanism-state in winners is consistent with INVERSE-BALDWIN-REPLICATES because the mechanism IS doing work in the winners; it's just not directed where selection rewards it." The intensified saturation aligns with the strengthened mechanism-axis signal from Interpretation (2), though row 3 does not fire per §28a.
4. **Guard 4 — GT-bypass artefact:** NOT TRIGGERED. n-exp GT_bypass_fraction mean = **0.006** at budget=5 (vs §2a's 0.007); pooled mean = 0.007. Far below 0.50.
5. **Guard 5 — δ-convergence artefact (universal-adapter in δ-space):** NOT TRIGGERED. δ_std on pooled n=40 = **2.590** (> 0.5); n-exp alone = 2.466; no convergent-δ collapse.
6. **Guard 6 — Adaptation-budget-too-high sanity at budget=5:** NOT VIOLATED on n-exp. `|δ_final|` at the physical ceiling = 5.000 (inherited from §2a pattern; explicitly verified on n-exp's 20 runs — no over-ceiling values).

**Infrastructure-fidelity check (principle 23/25, moved from v1 guard 7):** PASS on n-exp. `history.npz:initial_population_canonical_count == 0` across all 20 n-exp runs (verified via direct np.load inspection, Counter({0: 20})). Combined with §2a's 100/100 = 0 icc, pooled 120/120 = 0 across the entire §v2.5-plasticity-2a line.

### Falsifiability block

**Not applicable.** This chronicle does NOT propose, narrow, or broaden a tentative mechanism name per §16c. Same disposition as §v2.5-plasticity-2a: no mechanism-name candidate is advanced here. The row-3 prose-fit × F-lift-clause-fail pattern that intensifies on n=40 is a **candidate §2b grid-miss refinement** for the follow-up prereg to enumerate, not a mechanism name. Any mechanism-name introduction is deferred to the §2b follow-up prereg (which will chronicle the dedicated outcome row for the observed cell and may at that point propose a mechanism name with its own falsifiability block).

### Diagnostics (prereg-promise ledger — items from §v2.5-plasticity-2a prereg's "Diagnostics to log" block)

Per §v2.5-plasticity-2a prereg §Diagnostics-to-log (lines 380-391), items re-reported where n-exp contributes new data:

- **Per-seed × per-cell `F_AND_train`, `F_AND_test`:** budget=5 plastic: F_AND_test_plastic §2a=7/20, n-exp=7/20, pooled=14/40; F_AND_train_plastic §2a=2/20, n-exp=2/20, pooled=4/40; F_AND_test_frozen (unchanged) = 0/20.
- **R_fit_frozen_999, R_fit_plastic_999 per cell:** n-exp alone R_fit_plastic_999 = 0.065, pooled mean = 0.062 (vs §2a's 0.058); R_fit_frozen_999 unchanged at 0.000.
- **Per-individual `test_fitness_frozen`, `test_fitness_plastic`, `delta_final`, `has_gt`, `genotypes`:** emitted to 20 new `final_population.npz` files on the n-exp sweep per `dump_final_population=true`.
- **Per-cell `GT_bypass_fraction`:** n-exp 0.006; pooled 0.007. All < 0.50.
- **`Baldwin_gap` by Hamming bin {0, 1, 2, 3, ≥4}:** at sf=0.0 all non-GT-bypass individuals remain in h ≥ 4 (inherited structural property from §2a and §1a); `Baldwin_gap_h_ge4_mean` on n-exp = 0.3596 (vs §2a's 0.2546); pooled = 0.3071. Matches `max_gap_at_budget_5` definition (at sf=0.0 the h ∈ {2, 3, ≥4} max collapses to h=≥4 because other bins are empty).
- **`Baldwin_slope`:** NaN on n-exp (degenerate x-variance; same as §2a). Descriptive-only.
- **Per-cell `std(delta_final)` stratified by Hamming bin:** defined only for h ≥ 4 on n-exp; per-seed `delta_std_h*` values emitted in n-exp's `plasticity.csv`.
- **Per-cell seed-bootstrap 95% CI on `max_gap_at_budget_5` + `seed_majority` + `seed_minority_0_05`:** pooled n=40 values in Result table above.
- **Per-run `top1_winner_hamming` at budget=5:** all 20 n-exp runs return 5 (cap+1 sentinel; same as §2a). Pooled distribution: `{5: 40}`. CB classification: `n_cb_active=0`, `n_cb_inactive=40` (diagnostic only; block did not fire).
- **Per-cell paired `R_fit_plastic_999 − R_fit_frozen_999` on shared seeds (R_fit_delta_paired_sf0):** NOT computable for n-exp alone (no frozen control re-run on seeds 40..59 per prereg pre-commitment). §2a's R_fit_delta_paired_sf0 at budget=5 = +0.058 (on seeds 20..39) stands as the paired value.
- **Per-cell `|δ_final| ≥ 5` fraction split pop vs top-1 at budget=5:** n-exp pop 0.8614; top-1 17/20. Pooled top-1 32/40. Reported in Guard 3.
- **Per-cell best-of-run hex for top-1 winner per seed:** emitted per-run in n-exp's `sweep_index.json` (20 new `best_genotype_hex` entries). Every winner has `top1_winner_hamming = 5` → uniformly distant from canonical on active view, replicating §2a's attractor-category observation. Principle 21 inspection: the 37/40 seed_majority on `max_gap_at_budget_5` is not an F-solve attractor but a Baldwin-gap tail-occupancy attractor (stronger than §2a's 17/20).
- **Per-seed `initial_population_canonical_count` in gen-0:** 0/20 on n-exp (infrastructure PASS verified).
- **`Baldwin_slope` when defined:** all NaN on n-exp (same as §2a). Transparent §25 disclosure.

### Findings this supports / narrows

- **Narrows (EXISTING, NULL):** [findings.md#plasticity-narrow-plateau](findings.md#plasticity-narrow-plateau) — this n-expansion is a pre-committed continuation of §v2.5-plasticity-2a's follow-up data point on §v2.5-plasticity-1a's Falsifiability P-1 (shortcut-removal branch). Row 4 persistence on n=40 means **still neither clean support (row 1) nor clean refutation (row 3)** of the `selection-deception` diagnosis — the n-expansion pre-committed as the decision rule for row-4 ambiguity has landed, and its outcome (case ii: row 4 persists) dispatches to the §2b follow-up prereg. findings.md `plasticity-narrow-plateau` is NOT yet amended; the narrow/broaden call still awaits a dedicated outcome row in the follow-up prereg that can route cleanly.

### Next steps (from §v2.5-plasticity-2a chronicle's §2b follow-up commitment, now discharged to prereg-authoring phase)

The §v2.5-plasticity-2a chronicle's Next-steps bullet 5 (§2b follow-up commitment) and Interpretation (2) case (ii) pre-committed the following, now triggered by this chronicle's Row-4 persistence:

1. **§2b follow-up prereg required.** A new prereg on this axis must enumerate the (row-3-mechanism-signature × row-4-or-higher-F-lift) cell as a dedicated outcome row before interpreting. The signature to pre-register: `max_gap_at_budget_5` CI_lo ≥ 0.10 seed_majority ≥ 10 (or equivalently at n=40: CI_lo ≥ 0.10 seed_majority ≥ 20) AND δ_std > 2.0 AND frozen_F_prop ≤ 0.45 AND `0.15 < F-lift < 0.40` (mid-range) — which today has no pre-registered row. A dedicated row would also clarify the appropriate decision rule (EES viable? rank-2 ladder? a third mechanism family?) which this chronicle cannot pre-commit without a prereg round.
   - **Pre-committed inspection obligation for the §2b follow-up prereg (codex P2-1):** the 40/40 top1_winner_hamming=5 sentinel signature (reported in Interpretation (4)) must be explicitly enumerated as an axis the follow-up prereg grids, and inspection must include (a) direct winner-tape decoding to classify attractor structure at the active-view level (what the winners solve with, if not near-canonical), (b) a cap-raise check to reveal any structure in the 5..N range, (c) acknowledgment that this sentinel is NOT partial row-3 evidence under §28a — it is diagnostic-level evidence of "something structured persisting" that the mechanism-name candidate (if any is proposed in that prereg) would need to explain.
2. **DO NOT queue EES (§v2.5-plasticity-2b) or rank-2 (§v2.5-plasticity-1b) sweeps** — both remain blocked by the row-4 persistence (row 1 / row 3 pre-committed routes are not live). Neither finding the §v2.5-plasticity-1a `plasticity-narrow-plateau` should be amended.
3. **Parallel rank-2 engineering (VM implementation, 3–5 days) MAY continue** during the §2b prereg-amendment cycle — a separate engineering call the chronicle does not pre-commit (per §v2.5-plasticity-2a prereg verbatim); engineering does not imply queueing a rank-2 sweep.
4. **Theory.md / methodology review:** the mid-range F-lift × row-3-mechanism-signature combination is a candidate §26 axis-under-scoping case (row 4's "any" on max_gap/δ_std insufficient when the axis carries diagnostic-range signal). The §2b prereg-authoring work should consider whether this case deserves a new methodology entry (§26-extension or §2b-extension) beyond the already-flagged methodology-backlog item on `top1_winner_hamming` description/implementation drift from §v2.5-plasticity-2a.
