...
**p) Combined spatial+temporal specialization: mechanisms compose at peak but evolvability collapses on median.** `banded_phased.yaml` (§11.ab, 20 runs): new rule family with `n_bands=3, n_phases=2` tables indexed by (phase, band); 616-byte genotype with length-normalized mutation. Best 8-bit parity run: **0.980 = 251/256 correct** (5 errors only, best across 13 sweeps) — peaks higher than either banded_3 (0.969) or phased_2 (0.961) alone, so the two mechanisms DO add expressively. But median collapses to 0.691 ≈ uniform baseline with wide spread (0.67–0.98): at 616 bytes with ~3 flips/gen × 200 gens, most seeds fail to navigate the joint search space from random initialization. Same Goldilocks-zone pattern as per_row (§11.a-b, 1600 bytes) but less extreme — a few seeds find the combined potential. **The 8-bit parity ceiling is now an evolvability × expressiveness curve, not a pure expressive limit.** Spatial and temporal specialization are compatible but not additive under current search budget; reaching consistent full-solve would need larger pop/gens, an adaptive mutation schedule, or a warmer initial distribution. **Total runs across CA track: 558.**
