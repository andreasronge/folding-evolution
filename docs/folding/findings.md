...
Across twelve completed sweeps, only one intervention moves 8-bit parity's ceiling: spatial role-specialization (banded_3 from §11.a). Not bigger windows, not richer rules, not more search budget, not varied readouts, not richer families, not edge-of-chaos regimes. **Different rules in different rows** is the unique parity-mover; everything else is null. Mitchell/Crutchfield strengthened: CAs do density tasks well when given sufficient local integration, and parity is structurally resistant in ways that bigger local context cannot overcome.

**o) 🎯 Temporal specialization also breaks 8-bit parity — same magnitude as spatial.** `phase_schedule.yaml` (§11.b, 60 runs) added a new rule family `phased_ot`: `n_phases` separate OT rule tables plus a 16-entry schedule vector assigning each timestep to one phase. 8-bit parity: P=2 median **0.814 (best seed 0.961)**, P=3 median 0.805 — both roughly equal to banded_3's +0.11 lift from §11.a via a completely different mechanism. 7-bit majority: modest lift (0.91 → 0.92), much weaker than radius r=3's full solve. Error-structure of the best phased rule (0.961 = 246/256) distributes errors across bit-counts 3/4/5/7 (peak at 4), qualitatively different from banded_3's clean "all 8 errors at bit-count=5" — different internal computations reaching similar overall ceiling.

**Two independent specialization mechanisms — spatial (rows) and temporal (timesteps) — each move 8-bit parity from ~0.70 to ~0.80; neither alone reaches full solve.** Uniform dynamics (across any single dimension — state count, neighborhood size, search budget, readout shape, family) cannot lift the ceiling. Heterogeneity over *some* axis is the missing structural ingredient. Natural next probe: combine banded + phased to test whether the mechanisms add to a solved rule or saturate at the same ~0.80 bottleneck from different directions.

**p) Combined spatial+temporal specialization: mechanisms compose at peak but evolvability collapses on median.** `banded_phased.yaml` (§11.ab, 20 runs): new rule family with `n_bands=3, n_phases=2` tables indexed by (phase, band); 616-byte genotype with length-normalized mutation. Best 8-bit parity run: **0.980 = 251/256 correct** (5 errors only, best across 13 sweeps) — peaks higher than either banded_3 (0.969) or phased_2 (0.961) alone, so the two mechanisms DO add expressively. But median collapses to 0.691 ≈ uniform baseline with wide spread (0.67–0.98): at 616 bytes with ~3 flips/gen × 200 gens, most seeds fail to navigate the joint search space from random initialization. Same Goldilocks-zone pattern as per_row (§11.a-b, 1600 bytes) but less extreme — a few seeds find the combined potential.

**q) At pop=1024 the banded_phased median lifts to 0.824; expressive ceiling locks at ~0.97 across all specialization schemes.** `banded_phased_pop1024.yaml` (§11.ab-b, 10 runs — 9 complete after one MLX crash) reran banded_phased with 4× population. **Median lifts +0.13 (0.691 → 0.824), mean +0.10 (0.725 → 0.821)**; max essentially unchanged at 0.969. The typical seed now reliably matches banded_3 (0.805) and phased_2 (0.814) on median, confirming compatibility once evolvability is unblocked. But the combined family's peak doesn't exceed either single-specialization's peak (all three max-out in the 0.96–0.98 range). **The 8-bit parity ceiling at K=4 / N=16 / T=16 decomposes into:** (1) expressive bound ~0.97, reachable by heterogeneity in any single axis (spatial or temporal); (2) evolvability bound that couples genome length to required search budget — short genomes (~300 bytes) are evolvable at pop=256, 600-byte genomes need pop=1024; (3) full-solve bound (1.00) not reached under any tested configuration; the closest anyone got was 251/256 correct via banded_phased at pop=256. Further progress almost certainly requires scaling N, T, or K — not more heterogeneity at the current grid/step budget. **Total runs across CA track: 567.**
