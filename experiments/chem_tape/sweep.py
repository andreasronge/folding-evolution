#!/usr/bin/env python3
"""Generic sweep runner for chem-tape.
...
Expands `grid` into Cartesian-product overrides, merges with `base`, runs each
config into experiments/chem_tape/output/{sweep_name}/{hash}/. Resumable: skips
any config whose result.json already exists.
"""
...
def expand_grid(spec: dict[str, Any]) -> list[ChemTapeConfig]:
...
def main() -> int:
    ...
    output_root = args.output_root or Path("experiments/chem_tape/output") / sweep_name
    output_root.mkdir(parents=True, exist_ok=True)

    configs = expand_grid(spec)
    print(f"Sweep '{sweep_name}': {len(configs)} config(s) → {output_root}")

    to_run: list[ChemTapeConfig] = []
    skipped: list[ChemTapeConfig] = []
    for cfg in configs:
